::User.find(:all).each do |user|
  if user.plugins.where(:name => 'refinery_orders').blank?
    user.plugins.create(:name => "refinery_orders",
...
if defined?(::Page)
  ...
  thank_you_page = contact_us_page.children.create({
    ...
    :menu_match => "^/(orders|contact)/thank_you$",
    :show_in_menu => false,
    :deletable => false,
  ...
  thank_you_page.parts.create({
    :title => "Body",
    :body => "<p>We've received your order and will get back to you with a response shortly.</p><p><a href='/'>Return to the home page</a></p>",
    :position => 0
  })
